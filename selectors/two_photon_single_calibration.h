...
#ifndef two_photon_single_calibration_H
#define two_photon_single_calibration_H
...
class two_photon_single_calibration : public TGRSISelector
{
public:
        TGriffin *fGrif;
        ...
        two_photon_single_calibration(TTree * /*tree*/ = 0) : TGRSISelector(), fGrif(nullptr), fGriffinBgo(NULL), fScep(nullptr), fZDS(nullptr), fLaBr(nullptr), fLaBrBgo(NULL), fTAC(nullptr), fPaces(nullptr)
        {
                SetOutputPrefix("run");
        }
        virtual ~two_photon_single_calibration() {}
        ...
        ClassDef(two_photon_single_calibration, 1); // 2); //Makes ROOT happier
};
#endif

#ifdef two_photon_single_calibration_cxx
void two_photon_single_calibration::InitializeBranches(TTree *tree)
{
        if (!tree)
                return;
...
#endif // #ifdef two_photon_single_calibration_cxx
