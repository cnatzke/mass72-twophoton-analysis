...
void S1716Sept2019Selector::CreateHistograms()
{
	...
	fH2["singles_channel"] = new TH2D("singles_channel", "GRIFFIN crystal number vs #gamma energy", 65, 0., 65., gamma_bin_max / gamma_binning, gamma_bin_min, gamma_bin_max);
	...
	// gamma-beta coincidences
	fH2["singles_sum_beta"] = new TH2D("singles_sum_beta", "#beta-#gamma_{sum}, |#Deltat_{#gamma-#gamma}| < 30 ns ", 4000, 0, 4000, gamma_bin_max / gamma_binning, gamma_bin_min, gamma_bin_max);
	...
	// ADDBACK-ELECTRON matrices
	for (int i = 1; i < 6; i++)
	{
		fH2[Form("aeE_P%i", i)] = new TH2D(Form("aeE_P%i", i), Form("addback-e^{-} coincidences, PACES %i; GRIFFIN (keV); PACES (keV)", i), gbin, gx, gy, gbinPACES, gxPACES, gyPACES);
		fH2[Form("aeE_P%i_bg", i)] = new TH2D(Form("aeE_P%i_bg", i), Form("time-random addback-e^{-} coincidences, PACES %i; GRIFFIN (keV); PACES (keV)", i), gbin, gx, gy, gbinPACES, gxPACES, gyPACES);
		fH2[Form("aeE_P%i_z", i)] = new TH2D(Form("aeE_P%i_z", i), Form("addback-e^{-} coincidences, PACES %i, in #beta coincidence; GRIFFIN (keV); PACES (keV)", i), gbin, gx, gy, gbinPACES, gxPACES, gyPACES);
		fH2[Form("aeE_P%i_zbg", i)] = new TH2D(Form("aeE_P%i_zbg", i), Form("time-random addback-e^{-} coincidences, PACES %i, in #beta coincidence; GRIFFIN (keV); PACES (keV)", i), gbin, gx, gy, gbinPACES, gxPACES, gyPACES);
	}
...
void S1716Sept2019Selector::FillHistograms()
{
	...
	// Singles
	for (auto i = 0; i < singles_energy.size(); ++i)
	{
		fH2["singles_channel"]->Fill(singles_detector.at(i), singles_energy.at(i));

		for (auto j = 0; j < singles_energy.size(); j++)
		{
	...
	for (auto i = 0; i < addSuppr_CTen.size(); ++i)
	{
		...
		if (gotPaces)
		{
			for (auto j = 0; j < fPaces->GetMultiplicity(); ++j)
			{
				...
				if (CoincidenceCondition(addSuppr_time.at(i), fPaces->GetPacesHit(j)))
				{
					fH2[Form("aeE_P%i", fPaces->GetPacesHit(j)->GetDetector())]->Fill(addSuppr_CTen.at(i), fPaces->GetPacesHit(j)->GetEnergy());
				}
				if (BGCoincidenceCondition(addSuppr_time.at(i), fPaces->GetPacesHit(j)))
				{
					fH2[Form("aeE_P%i_bg", fPaces->GetPacesHit(j)->GetDetector())]->Fill(addSuppr_CTen.at(i), fPaces->GetPacesHit(j)->GetEnergy());
				}
			}
		}
			...
			for (auto k = 0; k < fZDS->GetMultiplicity(); ++k)
			{
				...
				if (gotPaces)
				{
					for (auto j = 0; j < fPaces->GetMultiplicity(); ++j)
					{
						...
						if (CoincidenceCondition(fPaces->GetPacesHit(j), fZDS->GetZeroDegreeHit(k)))
						{ // beta-tag condition on the electron
							if (CoincidenceCondition(addSuppr_time.at(i), fPaces->GetPacesHit(j)))
							{
								fH2[Form("aeE_P%i_z", fPaces->GetPacesHit(j)->GetDetector())]->Fill(addSuppr_CTen.at(i), fPaces->GetPacesHit(j)->GetEnergy());
							}
							if (BGCoincidenceCondition(addSuppr_time.at(i), fPaces->GetPacesHit(j)))
							{
								fH2[Form("aeE_P%i_zbg", fPaces->GetPacesHit(j)->GetDetector())]->Fill(addSuppr_CTen.at(i), fPaces->GetPacesHit(j)->GetEnergy());
							}
						}
					}
				}
