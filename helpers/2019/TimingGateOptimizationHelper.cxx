...
double twoPhotonCoincidenceTimeMax = 70.; // ns, max time diff for two-photon events
...
std::vector<Int_t> gammaGammaCoincidenceLimits = {30};
...
void TimingGateOptimizationHelper::CreateHistograms(unsigned int slot)
{
   ...
   fH1[slot]["zdsZdsTiming"] = new TH1D("zdsZdsTiming", "Diagnostic timing, ZDS - ZDS", 2000, -1000, 1000);
   ...
   {
      fH1[slot][Form("gammaSumBetaTimingDiagnostic%ins", myLimit)] = new TH1D(Form("gammaSumBetaTimingDiagnostic%ins", myLimit), Form("#gamma#gamma-#beta timing diagnostic, |#Deltat_{#gamma#gamma}| < %i ns", myLimit), 320, -160, 160);
      fH2[slot][Form("gammaSumBetaTiming%ins", myLimit)] = new TH2D(Form("gammaSumBetaTiming%ins", myLimit), Form("#Deltat #gamma#gamma-#beta, |#Deltat_{#gamma#gamma}| < %i ns", myLimit), 4000, -2000, 2000, gammaEnergyMax, gammaEnergyMin, gammaEnergyMax);
   }

   //* ------- Angles -------
   fH2[slot]["gammaSumAnglularMatrixBetaTagged"] = new TH2D("gammaSumAnglularMatrixBetaTagged", Form("Angle between #gamma#gamma (#beta-tagged), |#Deltat_{#gamma#gamma}| < %f ns", twoPhotonCoincidenceTimeMax), 180, 0, 180, gammaEnergyMax, gammaEnergyMin, gammaEnergyMax);
...
void TimingGateOptimizationHelper::Exec(unsigned int slot, TGriffin &grif, TGriffinBgo &grifBgo, TZeroDegree &zds)
{
   ...
   for (auto g1 = 0; g1 < grif.GetSuppressedMultiplicity(&grifBgo); ++g1)
   {
      ...
      fH1[slot].at("zdsMultiplicity")->Fill(zds.GetMultiplicity());
      for (int z1 = 0; z1 < zds.GetMultiplicity(); ++z1)
      {
         ...
         for (int z2 = z1 + 1; z2 < zds.GetMultiplicity(); ++z2)
         {
            ...
            Double_t zdsZdsTimeDiff = zds1->GetTime() - zds2->GetTime();
      ...
      for (auto g2 = g1 + 1; g2 < grif.GetSuppressedMultiplicity(&grifBgo); ++g2)
      {
         ...
         double gammaGammaTimeDiff = grif1->GetTime() - grif2->GetTime();
         ...
         for (Int_t myLimit : gammaGammaCoincidenceLimits)
         {
            ...
            if (TMath::Abs(gammaGammaTimeDiff) < myLimit)
            {
               fH1[slot].at(Form("gammaSumBetaTimingDiagnostic%ins", myLimit))->Fill(gammaGammaTimeDiff);
               ...
               for (auto z = 0; z < zds.GetMultiplicity(); ++z)
               {
                  ...
                  Double_t grif1BetaTimeDiff = zds1->GetTime() - grif2->GetTime();
                  ...
                  {
                     // time difference
                     fH2[slot].at(Form("gammaSumBetaTiming%ins", myLimit))->Fill(grif1BetaTimeDiff, sumEnergy);
                     fH2[slot].at(Form("gammaSumBetaTiming%ins", myLimit))->Fill(grif2BetaTimeDiff, sumEnergy);
                  } // end angle restriction
               }    // end beta tag
            }       // end coincidence loop
         }          // end gammaGammaCoincidenceLimits

         // Timing optimization - 70 ns
         if (TMath::Abs(gammaGammaTimeDiff) < twoPhotonCoincidenceTimeMax)
         {
            auto sumEnergy = calibratedEnergyGrif1 + calibratedEnergyGrif2;
            fH1[slot].at("gammaSumCoincidenceTiming")->Fill(gammaGammaTimeDiff);
            ...
            for (auto z2 = 0; z2 < zds.GetMultiplicity(); ++z2)
            {
               ...
               fH2[slot].at("gammaSumAnglularMatrixBetaTagged")->Fill(angle, sumEnergy);

               Double_t grif1BetaTimeDiff = zds1->GetTime() - grif1->GetTime();
               Double_t grif2BetaTimeDiff = zds1->GetTime() - grif2->GetTime();

               // time difference
               fH2[slot].at("gammaSumBetaTiming")->Fill(grif1BetaTimeDiff, sumEnergy);
               fH2[slot].at("gammaSumBetaTiming")->Fill(grif2BetaTimeDiff, sumEnergy);
