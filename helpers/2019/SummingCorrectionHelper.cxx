#include "SummingCorrectionHelper.hh"
...
double gammaMinimumEnergy = 0;   // keV
...
void SummingCorrectionHelper::CreateHistograms(unsigned int slot)
{
   //* ------- Charge -------
...
void SummingCorrectionHelper::Exec(unsigned int slot, TGriffin &grif, TGriffinBgo &grifBgo, TZeroDegree &zds)
{
   ...
   for (auto g1 = 0; g1 < grif.GetMultiplicity(); ++g1)
   {
      ...
      fH2[slot].at("gammaEnergyChannelUnsuppressed")->Fill(grif1->GetArrayNumber(), calibratedEnergyGrif1);
      ...
      for (auto g2 = g1 + 1; g2 < grif.GetMultiplicity(); ++g2)
      {
         ...
         if (angle > 178 && angle < 182)
         {
            ...
            if (ggTime < ggHigh)
            {
               fH1[slot].at("sumCorrectionsTimingDiagnostic")->Fill(ggTime);
               fH2[slot].at("sumCorrectionsMatrixUnsuppressed")->Fill(calibratedEnergyGrif1, calibratedEnergyGrif2);
            }
            else if (bgLow < ggTime && ggTime < bgHigh)
            {
               fH2[slot].at("sumCorrectionsMatrixUnsuppressed")->Fill(calibratedEnergyGrif1, calibratedEnergyGrif2, -ggHigh / (bgHigh - bgLow));
            }
         } // -- end angle > 170
      }    // -- end second gamma
   ...
   for (auto g1 = 0; g1 < grif.GetSuppressedMultiplicity(&grifBgo); ++g1)
   {
      ...
      fH2[slot].at("hpgeChargeChannel")->Fill(grif1->GetArrayNumber(), grif1->GetCharge());
      ...
      for (auto g2 = g1 + 1; g2 < grif.GetSuppressedMultiplicity(&grifBgo); ++g2)
      {
         ...
         if (angle > 178 && angle < 182)
         {
            ...
            {
               fH2[slot].at("sumCorrectionsMatrix")->Fill(calibratedEnergyGrif1, calibratedEnergyGrif2, -ggHigh / (bgHigh - bgLow));
            }
...
Addback_t SummingCorrectionHelper::GetAddback(TGriffin &grif, TGriffinBgo &grifBgo)
{
...
bool SummingCorrectionHelper::HasDuplicate(const std::vector<int> &vec)
{
