...
void AngularCorrelationHelper::CreateHistograms(unsigned int slot)
{
   ...
   for(int i = 0; i < static_cast<int>(fAngleCombinations.size()); ++i) {
      fH2[slot][Form("gammaGamma%d", i)] = new TH2D(
         Form("gammaGamma%d", i),
         Form("%.1f^{o}: #gamma-#gamma, |#Deltat_{#gamma-#gamma}| < %.1f", fAngleCombinations[i].first, ggHigh), 2000,
         0., 2000., 2000, 0., 2000.);
      ...
      fH2[slot][Form("gammaGammaBG%d", i)] =
         new TH2D(Form("gammaGammaBG%d", i),
                  Form("%.1f^{o}: #gamma-#gamma, #Deltat_{#gamma-#gamma} = %.1f - %.1f", fAngleCombinations[i].first,
                       bgLow, bgHigh),
                  2000, 0., 2000., 2000, 0., 2000.);
   ...
   for(int i = 0; i < static_cast<int>(fAngleCombinationsAddback.size()); ++i) {
      fH2[slot][Form("addbackAddback%d", i)] =
         new TH2D(Form("addbackAddback%d", i),
                  Form("%.1f^{o}: #gamma-#gamma with addback, |#Deltat_{#gamma-#gamma}| < %.1f",
                       fAngleCombinationsAddback[i].first, ggHigh),
                  2000, 0., 2000., 2000, 0., 2000.);
      fH2[slot][Form("addbackAddbackBeta%d", i)] =
         new TH2D(Form("addbackAddbackBeta%d", i),
                  Form("%.1f^{o}: #gamma-#gamma with addback, |#Deltat_{#gamma-#gamma}| < "
      ...
      fH2[slot][Form("addbackAddbackBG%d", i)] =
         new TH2D(Form("addbackAddbackBG%d", i),
                  ...
                  2000, 0., 2000., 2000, 0., 2000.);
      fH2[slot][Form("addbackAddbackBetaBG%d", i)] =
         new TH2D(Form("addbackAddbackBetaBG%d", i),
                  Form("%.1f^{o}: #gamma-#gamma with addback, #Deltat_{#gamma-#gamma} = %.1f "
                       "- %.1f, #Deltat_{#gamma-#beta} = %.1f - %.1f",
                       fAngleCombinationsAddback[i].first, bgLow, bgHigh, gbLow, gbHigh),
                  2000, 0., 2000., 2000, 0., 2000.);
   }
   fH2[slot]["gammaGamma"] = new TH2D("gammaGamma", Form("#gamma-#gamma, |#Deltat_{#gamma-#gamma}| < %.1f", ggHigh),
                                      2000, 0., 2000., 2000, 0., 2000.);
   fH2[slot]["gammaGammaBeta"] =
      new TH2D("gammaGammaBeta",
               Form("#gamma-#gamma, |#Deltat_{#gamma-#gamma}| < %.1f, #Deltat_{#gamma-#beta} = %.1f - %.1f", ggHigh,
                    gbLow, gbHigh),
               2000, 0., 2000., 2000, 0., 2000.);
   ...
   for(int i = 0; i < static_cast<int>(fAngleCombinations.size()); ++i) {
      fH2[slot][Form("gammaGammaMixed%d", i)] =
         new TH2D(Form("gammaGammaMixed%d", i), Form("%.1f^{o}: #gamma-#gamma", fAngleCombinations[i].first), 2000, 0.,
                  2000., 2000, 0., 2000.);
      fH2[slot][Form("gammaGammaBetaMixed%d", i)] =
         new TH2D(Form("gammaGammaBetaMixed%d", i),
                  Form("%.1f^{o}: #gamma-#gamma, #Deltat_{#gamma-#beta} = %.1f - %.1f", fAngleCombinations[i].first,
                       gbLow, gbHigh),
                  2000, 0., 2000., 2000, 0., 2000.);
   }
   for(int i = 0; i < static_cast<int>(fAngleCombinationsAddback.size()); ++i) {
   ...
   fH2[slot]["addbackAddbackBetaMixed"] =
      new TH2D("addbackAddbackBetaMixed",
               ...
               2000, 0., 2000.);
   // plus hitpatterns for gamma-gamma and beta-gamma for single crystals
   fH2[slot]["gammaGammaHPMixed"] =
      new TH2D("gammaGammaHPMixed", "#gamma-#gamma hit pattern", 65, 0., 65., 65, 0., 65.);
   ...
   for(auto g1 = 0; g1 < grif.GetMultiplicity(); ++g1) {
      ...
      for(auto g2 = 0; g2 < grif.GetMultiplicity(); ++g2) {
         ...
         if(ggTime < ggHigh) {
            ...
            if(coincBeta) {
               ...
               fH2[slot]
                  .at(Form("gammaGammaBetaBG%d", angleIndex->second))
      ...
      for(auto g2 = 0; g2 < fLastGrif.GetMultiplicity(); ++g2) {
         ...
         if(coincBeta) {
            ...
            fH2[slot]
               .at(Form("gammaGammaBetaMixed%d", angleIndex->second))
               ->Fill(grif1->GetEnergy(), grif2->GetEnergy());
         }
      }
   }
